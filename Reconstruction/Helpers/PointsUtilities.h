...
namespace Helpers {
	...
	class PointUtilities
	{
		...
		public:
			...
			float GetMaxAbsCoord(vector<vector<Point3f>> points, string coordinate, bool excludeZero = false);
			float GetMaxAbsCoord(vector<Point3f> points, string coordinate, bool excludeZero = false);
			float GetMaxAbsCoord(vector<Vec6f> points);
			float GetMaxAbsCoord(vector<Vec<Point3f, 3>> points);
			float GetMaxAbsCoord(Mat image, string coordinate, CameraProperties calib, bool excludeZero = false);
			float GetMaxAbsCoord(vector<float> points, bool excludeZero = false);
			float GetMinAbsCoord(Mat image, string coordinate, CameraProperties calib, bool excludeZero = false);
			float GetMinAbsCoord(vector<Point3f> points, string coordinate, bool excludeZero = false);
			float GetMinAbsCoord(vector<vector<Point3f>> points, string coordinate, bool excludeZero = false);
			float GetMinAbsCoord(vector<float> points, bool excludeZero = false);
