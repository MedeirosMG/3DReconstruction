...
namespace Services {
	...
	class ReconstructionCompareService
	{
		...
		public:
			...
			void HeartComparison(vector<Point3f> reconstructionPoints, CameraProperties calib, Mat mapImage, string pathExport, std::map<string, double>* batchResult);
			void Execute(vector<Point3f> reconstructionPoints, CameraProperties calib, Mat mapImage, string pathExport, std::map<string, double>* batchResult = NULL);
	};
